...
Elm.Main.make = function (_elm) {
   ...
   var targetSelectionStart = A2($Json$Decode.map,$Basics.toString,A2($Json$Decode.at,_U.list(["target","selectionStart"]),$Json$Decode.$int));
   var view = F2(function (address,string) {
      ...
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.placeholder("Text to reverse")
                      ...
                      ,A3($Html$Events.on,"click",targetSelectionStart,function (x) {    return A2($Signal.message,address,A2($Debug.log,"pos",x));})
                      ,myStyle]),
              _U.list([]))
              ,A2($Html.div,_U.list([myStyle]),_U.list([$Html.text(string)]))]));
   });
   var main = $StartApp$Simple.start({model: "",view: view,update: update});
   return _elm.Main.values = {_op: _op,main: main,targetSelectionStart: targetSelectionStart,update: update,view: view,myStyle: myStyle};
};
