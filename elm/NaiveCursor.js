...
Elm.Cursor = Elm.Cursor || {};
Elm.Cursor.make = function (_elm) {
   "use strict";
   _elm.Cursor = _elm.Cursor || {};
   if (_elm.Cursor.values) return _elm.Cursor.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getC = function (c1) {    return A2($Focus.get,c1.lens,c1.state);};
   var updateC = F2(function (c1,f) {    return A2($Signal.message,c1.receiver,A3($Focus.update,c1.lens,f,c1.state));});
   var setC = F2(function (c1,v) {    return A2($Signal.message,c1.receiver,A3($Focus.set,c1.lens,v,c1.state));});
   var idL = A2($Focus.create,$Basics.identity,F2(function (f,r) {    return f(r);}));
   var createC = F2(function (mailbox,state) {    return {receiver: mailbox.address,state: state,lens: idL,signal: mailbox.signal};});
   var drawC = F2(function (state,view) {
      var mailbox = $Signal.mailbox(state);
      var cursorC_ = createC(mailbox);
      return A2($Signal.map,function (_p0) {    return view(cursorC_(_p0));},mailbox.signal);
   });
   _op[">=>"] = F2(function (c1,f1) {    return _U.update(c1,{lens: A2($Focus._op["=>"],c1.lens,f1)});});
   var Cursor = F4(function (a,b,c,d) {    return {receiver: a,state: b,lens: c,signal: d};});
   return _elm.Cursor.values = {_op: _op,createC: createC,drawC: drawC,getC: getC,setC: setC,updateC: updateC,Cursor: Cursor};
};
Elm.NaiveCursor = Elm.NaiveCursor || {};
Elm.NaiveCursor.make = function (_elm) {
   "use strict";
   _elm.NaiveCursor = _elm.NaiveCursor || {};
   if (_elm.NaiveCursor.values) return _elm.NaiveCursor.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Automaton = Elm.Automaton.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Cursor = Elm.Cursor.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Focus = Elm.Focus.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "dbname" in v && "lastSample" in v ? {_: {}
                                                                              ...
                                                                              v.lastSample)} : _U.badPort("an object with fields `dbname`, `lastSample`",v);
      })) : _U.badPort("an array",v);
   });
   var topFiveQueriesL = A2($Focus.create,
   function (_) {
      return _.topFiveQueries;
   },
   F2(function (f,r) {    return _U.update(r,{topFiveQueries: f(r.topFiveQueries)});}));
   var nbQueriesL = A2($Focus.create,function (_) {    return _.nbQueries;},F2(function (f,r) {    return _U.update(r,{nbQueries: f(r.nbQueries)});}));
   var countClassNameL = A2($Focus.create,
   function (_) {
      return _.countClassName;
   },
   F2(function (f,r) {    return _U.update(r,{countClassName: f(r.countClassName)});}));
   var lastSampleL = A2($Focus.create,function (_) {    return _.lastSample;},F2(function (f,r) {    return _U.update(r,{lastSample: f(r.lastSample)});}));
   var dbnameL = A2($Focus.create,function (_) {    return _.dbname;},F2(function (f,r) {    return _U.update(r,{dbname: f(r.dbname)});}));
   var queryL = A2($Focus.create,function (_) {    return _.query;},F2(function (f,r) {    return _U.update(r,{query: f(r.query)});}));
   var formatElapsedL = A2($Focus.create,
   function (_) {
      return _.formatElapsed;
   },
   F2(function (f,r) {    return _U.update(r,{formatElapsed: f(r.formatElapsed)});}));
   var elapsedClassNameL = A2($Focus.create,
   function (_) {
      return _.elapsedClassName;
   },
   F2(function (f,r) {    return _U.update(r,{elapsedClassName: f(r.elapsedClassName)});}));
   var viewTopFiveQueries = function (query) {
      var cursor1 = query;
      var elapsedClassName = A2($Cursor._op[">=>"],cursor1,elapsedClassNameL);
      var formatElapsed = A2($Cursor._op[">=>"],cursor1,formatElapsedL);
      var query1 = A2($Cursor._op[">=>"],cursor1,queryL);
      return A2($Html.td,
      _U.list([$Html$Attributes.$class(A2($Basics._op["++"],"Query ",$Cursor.getC(elapsedClassName)))]),
      _U.list([$Html.text($Cursor.getC(formatElapsed))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("popover left")]),
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("popover-content")]),_U.list([$Html.text($Cursor.getC(query1))]))
                      ,A2($Html.div,_U.list([$Html$Attributes.$class("arrow")]),_U.list([]))]))]));
   };
   var viewDatabase = function (database) {
      var cursor = database;
      var dbname = A2($Cursor._op[">=>"],cursor,dbnameL);
      var lastSample = A2($Cursor._op[">=>"],cursor,lastSampleL);
      var countClassName = A2($Cursor._op[">=>"],lastSample,countClassNameL);
      var nbQueries = A2($Cursor._op[">=>"],lastSample,nbQueriesL);
      var topFiveQueries = A2($Cursor._op[">=>"],lastSample,topFiveQueriesL);
      return A2($Html.tr,
      _U.list([]),
      A2($Basics._op["++"],
      _U.list([A2($Html.td,_U.list([$Html$Attributes.$class("dbname")]),_U.list([$Html.text($Cursor.getC(dbname))]))
              ,A2($Html.td,
              _U.list([$Html$Attributes.$class("query-count")]),
              _U.list([A2($Html.span,
              _U.list([$Html$Attributes.$class($Cursor.getC(countClassName))]),
              _U.list([$Html.text($Basics.toString($Cursor.getC(nbQueries)))]))]))]),
      A2($List.map,function (q) {    return viewTopFiveQueries(A2($Cursor.createC,$Signal.mailbox(q),q));},$Cursor.getC(topFiveQueries))));
   };
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.tbody,_U.list([]),A2($List.map,viewDatabase,model))]))]));
   });
   ...
   var db2c = function (database) {    return A2($Cursor.createC,$Signal.mailbox(database),database);};
   var dbList2cLIst = function (dbList) {    return A2($List.map,db2c,dbList);};
   var autodbList2cLIst = $Automaton.pure(dbList2cLIst);
   var initialModel = _U.list([]);
   var dbnew = A2($List.map,db2c,initialModel);
   var actions2 = A3($Automaton.run,autodbList2cLIst,dbnew,dispatchGenerateData);
   var actions = A2($Signal.map,LoadData,actions2);
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: dbnew,_1: $Effects.none},update: update,view: view,inputs: _U.list([actions])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var Database = F2(function (a,b) {    return {dbname: a,lastSample: b};});
   var LastSample = F3(function (a,b,c) {    return {countClassName: a,nbQueries: b,topFiveQueries: c};});
   var Query = F3(function (a,b,c) {    return {elapsedClassName: a,formatElapsed: b,query: c};});
   return _elm.NaiveCursor.values = {_op: _op
                                    ,Query: Query
                                    ...
                                    ,dbnew: dbnew
                                    ,db2c: db2c
                                    ,autodbList2cLIst: autodbList2cLIst
                                    ,dbList2cLIst: dbList2cLIst
                                    ,actions2: actions2
                                    ,LoadData: LoadData
                                    ,update: update
                                    ,elapsedClassNameL: elapsedClassNameL
                                    ,formatElapsedL: formatElapsedL
                                    ,queryL: queryL
                                    ,viewTopFiveQueries: viewTopFiveQueries
                                    ,dbnameL: dbnameL
                                    ,lastSampleL: lastSampleL
                                    ,countClassNameL: countClassNameL
                                    ,nbQueriesL: nbQueriesL
                                    ,topFiveQueriesL: topFiveQueriesL
