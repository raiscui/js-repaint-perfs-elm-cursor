...
Elm.Opt.make = function (_elm) {
   ...
   var viewTopFiveQueries = function (query) {
      return A2($Html.td,
      _U.list([$Html$Attributes.$class(A2($Basics._op["++"],"Query ",query.elapsedClassName))]),
      _U.list([$Html.text(query.formatElapsed)
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("popover left")]),
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("popover-content")]),_U.list([$Html.text(query.query)]))
   ...
   var viewDatabase = function (database) {
      ...
      _U.list([]),
      A2($Basics._op["++"],
      _U.list([A2($Html.td,_U.list([$Html$Attributes.$class("dbname")]),_U.list([$Html.text(database.dbname)]))
              ,A2($Html.td,
      ...
      A2($List.map,$Html$Lazy.lazy(viewTopFiveQueries),database.lastSample.topFiveQueries)));
   };
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.tbody,_U.list([]),A2($List.map,$Html$Lazy.lazy(viewDatabase),model))]))]));
   });
   ...
   var actions = _U.list([A2($Signal.map,LoadData,dispatchGenerateData)]);
   var initialModel = _U.list([]);
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: initialModel,_1: $Effects.none},update: update,view: view,inputs: actions});
   var main = app.html;
